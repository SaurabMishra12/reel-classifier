...
export default function App() {
  ...
  const handleAutoClassifyAndSave = async (url) => {
    ...
    try {
      ...
      const result = await classifyText(captionText);
      
      // Save the reel automatically
      await saveReel(url, captionText, result);
      
      Alert.alert(
        'Reel Saved! 🎉', 
        `Your reel has been automatically categorized as "${result}" and saved to your library.`,
        [
          { text: 'OK' }, 
